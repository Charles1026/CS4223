...
In this assignment, I used SimpleScalar, a simulation software for RISC computer architecture to deepen my understanding of ILP. Based on my student number, I will be working on the compress benchmark. For the first question, I found that the highest performance processor with an area less than or equal to `60` units has an IPC of `1.186` with a pipeline width of 2, 2 integer arithmetic logic units(ALUs), 1 integer multiplier, 1 floating point(FP) ALU, 1 FP multiplier, out-of-order issue with a 16 unit reorder buffer and 8 unit load store queue. For the second question, I found that the best performance per watt value processor is actually the minimum spec processor with IPC/Watt of `0.0420`, pipeline width of 1, 1 integer arithmetic logic units(ALUs), 1 integer multiplier, 1 FP ALU, 1 FP multiplier and in-order issue. For the third question I found that the best l1 cache config for the highest IPC performance processor found in question 1 has `128` sets of data and instruction l1 cache each. 
...
First, I wanted to get orient myself with the IPC of the default configuration. Thus, I ran the compress benchmark with the default configuration with area of `106.08`, as seen below, to obtain an IPC of `1.6364`. As the area for the default configuration is too large, we have to decrease it to below 60 whilst minimally impacting our IPC performance.
...
Next, we have to understand the profile of our program, mainly the frequencies of instruction classes being run. Hence, we use the command `srun sim-profile -iclass benchmarks/compress95.ss < benchmarks/compress95.in > benchmarks/compress95.out` below to obtain the summary seen below.
...
Furthermore, I predict that we will need out-of-order issue as there are alot of load/store operations which have a long downtime on average, so having the ability to issue with other non true dependent instructions whilst waiting would greatly help our IPC. As such we should see out-of-order issue with a decently sized Load/Store Queue and Reorder Buffer in our ideal solution. In addition, since both the load/store operations and integer computation use the integer ALU, it will be more favoured than the integer multiplication unit in our ideal solution.
...
Although not the exact code used in our benchmark, it is reasonable to infer from the above that during each compression / decompression stage, the program loops through relatively few lines of code for many iterations whilst parsing alot of data. Hence, my inital assumption would be that there would be alot more sets allocated to data as compared to instruction in the l1 cache for the ideal configuration, which would probably be configuration 6 or 7.
