...
if __name__ == "__main__":
  ...
  constrained_config_values = []
  constrained_ipc = []
  constrained_ipc_per_watts = []
  # Open result log file
  with open("./res_2.log", "a") as res_file:
    ...
    for state, area, area_by_part in (iter := tqdm.tqdm(zip(constrained_states, constrained_areas, constrained_areas_by_part))):
      ...
      constrained_config_values.append(config_values)
      ...
      if match:
        ...
        ipc = float(splited[1])
        ipc_per_watt = ipc / area
        res_file.write(f"Configuration: {config_values}; Area per resource: {area_by_part}; Total Area: {area}; IPC: {ipc}; IPC/Watt: {ipc_per_watt}\n")
        res_file.flush()
        constrained_ipc.append(ipc)
        constrained_ipc_per_watts.append(ipc_per_watt)
  ...
  constrained_ipc_per_watts = np.asarray(constrained_ipc_per_watts)
  best_performing_idx = np.argmax(constrained_ipc_per_watts)
  print(f"Best performing state(idx {best_performing_idx}) is: {constrained_config_values[best_performing_idx]}, "\
    f"with area {constrained_areas[best_performing_idx]} and IPC of {constrained_ipc[best_performing_idx]} and IPC/Watt of {constrained_ipc_per_watts[best_performing_idx]}.")
